...
/// Generate a portable C firmware descriptor and a dependency-free Python host helper.
pub fn generate_firmware_bundle(
    ...
    fs::write(output_dir.join(files[4]), render_host_python(&pins))?;
...
pub fn firmware_bundle_schema() -> Value {
    json!({
        ...
        "required": ["schema_version", "mcu_reference", "mcu_value", "pins", "files", "c_build", "python_check"],
        "properties": {
