...
def generate_skidl(spec: Mapping[str, Any], *, include_netlist: bool = True) -> str:
    ...
    electrical_json = json.dumps(
        {
            "parts": {part["reference"]: part["electrical"] for part in parts},
            "nets": {net["name"]: {"voltage_v": net["voltage_v"]} for net in nets},
        },
        ensure_ascii=False,
        separators=(",", ":"),
        sort_keys=True,
    ...
    lines = ["# Generated by pcbex Text-to-Circuit; do not edit by hand.",
             ...
             f"PCBEX_ELECTRICAL_JSON = {_quote(electrical_json)}", ""]
    for net in nets:
