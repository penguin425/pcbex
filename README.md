...
| Registry threshold governance v1 | Root-authorize a fixed authority set, require distinct-key threshold approval for every organization decision, and lock out root-only bypasses after activation | `pcbex factory-release-state-transparency-external-gossip-organization-registry-threshold-governance-verification-report-schema` |
| Registry governance rotation v1 | Change governance membership, keys, or threshold only when both retained and successor quorums approve one state-bound next generation | `pcbex factory-release-state-transparency-external-gossip-organization-registry-governance-rotation-verification-report-schema` |
...
| Parallel final-checkpoint acquisition v1 | Acquire witnesses concurrently, retain partial failures, and replay the quorum offline | [Guide](docs/FACTORY_RELEASE_FINAL_CHECKPOINT_WITNESS_RECEIPT_QUORUM_CHECKPOINT_WITNESS_QUORUM_ACQUISITION.md) |
| Final-checkpoint receipt transparency v1 | Normalize one v1.526 receipt into the witnessed approval-log chain | [Guide](docs/FACTORY_RELEASE_FINAL_CHECKPOINT_WITNESS_RECEIPT_QUORUM_CHECKPOINT_WITNESS_RECEIPT_TRANSPARENCY.md) |
| Final-checkpoint receipt admission v1 | Replay identity, freshness, trust, response, checkpoint, and log evidence before append | [Guide](docs/FACTORY_RELEASE_FINAL_CHECKPOINT_WITNESS_RECEIPT_QUORUM_CHECKPOINT_WITNESS_RECEIPT_ADMISSION.md) |
| Final-checkpoint receipt admission quorum v1 | Verify distinct receipts and bind the sorted log suffix plus complete log | [Guide](docs/FACTORY_RELEASE_FINAL_CHECKPOINT_WITNESS_RECEIPT_QUORUM_CHECKPOINT_WITNESS_RECEIPT_QUORUM_ADMISSION.md) |
