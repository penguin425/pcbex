...
Version 1.463.0 adds the explicit downstream
[`generate-circuit-kicad-board`](CIRCUIT_KICAD_BOARD_WRITER.md) producer. It
consumes an approved circuit/schematic pair, a complete embedded footprint
closure, and explicit construction/physical profiles, then creates a new
placed-but-unrouted board and requires that same board-binding gate to approve
the exact generated bytes. It does not discover libraries, route copper, run
native DRC/DFM, or authorize manufacturing or procurement.
Version 1.464 adds a later, explicit
[`build-procurement-intent`](PROCUREMENT_INTENT.md) consumer for a
catalog-backed retained generation bundle. It fully replays the exact snapshot
selection and requires its populated reference set, footprints, and MPNs to
match the resolved circuit and Rust-verified final BOM for one exact board and
manufacturing ZIP; values are compared between the final BOM and resolved
circuit because catalog selections have no value field.
This does not extend the generation bundle or make the bundle a board
electrical-binding artifact. The BOM comparison does not compare nets, pins,
or connectivity; the package manifest's input basename is not approval-bound.
Approved line quantities are populated-reference counts for one board only.
The adapter performs no network request, establishes no current stock or
supplier authenticity, and neither authorizes procurement nor places an order.
