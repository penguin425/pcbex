...
By default it runs a strict C11 compile/link/smoke test using `cc` and Python
syntax plus self-tests using `python3`. `--skip-build` is available for source
generation-only workflows, but leaves the manifest with both build gates marked
as not attempted. `pcbex firmware-schema` prints the manifest schema.
