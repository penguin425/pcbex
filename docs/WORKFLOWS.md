...
| Quorum-bound factory receipt log and report | Restricted checkpoint signing | validate canonical met quorum → verify complete log chain and exact ID/count/head/digest → match every ordered factory-receipt suffix event → read private key → emit the existing generic checkpoint |
| Exact factory receipt-quorum evidence | Domain-separated checkpoint | repeat the strict quorum/log gate → hash normalized report → bind registry/checkpoint/threshold/log/signer → sign under the factory-specific domain → re-read inputs → verify with the dedicated trusted key |
| Dedicated factory receipt-quorum checkpoint | Independent checkpoint witness quorum | each witness re-verifies exact report/log/checkpoint evidence → sign under a separate witness domain → pair direct trusted keys → reject stale, repeated, or signer-role-colliding keys → retain the canonical threshold result |
| Dedicated checkpoint witnesses plus successor keys | Rotated checkpoint-witness quorum trust | initialize identity-bound trust → old/new dual-sign one adjacent transition → apply to a new retained snapshot → pair witnesses with current trust states → reject stale keys and mixed trust modes |
...
| Quorum-bound checkpoint-witness receipt log and report | Restricted checkpoint signing | validate canonical met quorum → verify complete admission-log chain and exact ID/count/head/digest → match every ordered checkpoint-witness receipt suffix event → read private key → emit the existing generic checkpoint |
| Quorum-bound checkpoint-witness receipt log and report | Domain-separated checkpoint | validate exact v1.512 report/log/suffix → hash normalized report → bind registry and prior factory checkpoint → sign threshold and complete admission-log state under the new domain → verify with a dedicated trusted key |
| Dedicated checkpoint-witness receipt-quorum checkpoint | Independent checkpoint witness quorum | each witness re-verifies the exact v1.512 report, admission log, and v1.514 checkpoint → sign under a separate witness domain → pair direct trusted keys → reject stale, repeated, weak, or signer-role-colliding keys → retain the canonical threshold result |
| Final checkpoint witnesses plus successor keys | Rotated final-witness quorum trust | initialize identity-bound trust → old/new dual-sign one adjacent transition → apply a new retained snapshot → pair unchanged v1.515 witnesses with current trust states → reject stale keys and mixed trust modes |
| Final checkpoint plus remote witness endpoints | Remotely acquired final-witness quorum | verify exact v1.512 report/log and v1.514 checkpoint locally → POST bounded public evidence → verify one canonical v1.515 response with a direct pin or current v1.516 trust → retain witness plus replayable receipt → run the unchanged quorum verifier |
...
| Exact final-receipt quorum evidence | Domain-separated final checkpoint | repeat the exact v1.521 gate → hash the normalized report → bind both prior factory checkpoints, threshold, complete final-admission log, and signer → sign under the dedicated final domain → re-read every input → verify with an independently pinned key |
| Dedicated final checkpoint plus independent witness keys | Final-checkpoint witness quorum | re-run the exact v1.521/v1.523 verification path → sign the checkpoint chain under the final-witness domain → enforce freshness, distinct IDs and keys, threshold, and checkpoint-signer role separation |
| Final-checkpoint witnesses plus successor keys | Generation-chained final-witness trust | initialize identity-bound trust → old/new dual-sign one adjacent transition → apply a predecessor-linked retained snapshot → pair unchanged v1.524 witnesses with current trust states → reject stale keys and mixed trust modes |
| Canonical remote final receipt-quorum checkpoint-witness receipt | Signed final-checkpoint receipt transparency | validate the closed v1.526 receipt → normalize exact receipt/checkpoint/request/response/witness bindings → append a new log snapshot → sign and verify the exact head |
| Retained final receipt-quorum checkpoint-witness evidence | Verifier-bound final-checkpoint receipt admission | replay exact v1.521 report/log and v1.523 checkpoint → reconstruct the identity- and trust-bound request → match the raw v1.524 response → re-verify signature and admission-time freshness → append the unchanged v1.528 event |
| Retained final receipt-quorum checkpoint-witness receipt set | Verifier-bound final-checkpoint receipt quorum admission | verify shared v1.521/v1.523 evidence once → reconstruct each distinct identity/trust-bound request → replay exact v1.524 responses → run the production witness quorum once → append the sorted unchanged v1.528 suffix → bind the complete log report |
