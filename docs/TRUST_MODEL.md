...
| Factory-release external-gossip organization registry | One independently pinned authority key signed the complete selected-ledger registry history; every selected quorum member belongs to an active organization and matches an admitted exact latest v1.492 observer trust-state digest | Authority threshold governance or key rotation, host-ledger rollback resistance, global non-equivocation, trusted time, real organizational independence or key custody, factory/legal identity, order, payment, or exactly-once execution |
