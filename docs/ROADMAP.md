...
The released v1.493.0 milestone adds an independently pinned generation-zero
...
The released v1.494.0 milestone makes that operational authority replaceable
without changing any v1.493 wire artifact. Each handoff advances the shared
registry generation exactly once, names the preceding transition-or-rotation
digest, preserves organizations and admissions, and requires Ed25519
signatures from both the current and successor keys over one payload.

The selected-ledger loader rejects historical authority-key reuse, observer
role collisions, gaps, and competing transition/rotation records. Both mutation
commands serialize on the descriptor-pinned ledger manifest before guarded
no-replace publication, while the new self-contained report replays typed mixed
history from the independently pinned genesis. The v1.493 verifier fails closed
when it encounters a rotation.

Authority threshold governance, host-ledger rollback resistance, trusted time,
...
threshold, and issue time under a dedicated Ed25519 signature.

Each threshold transition embeds that policy and requires ordered approvals
from distinct configured keys over one action, exact next generation, prior
history digest, governance digest, optional current observer admission, reason,
and nondecreasing time. The first accepted event stores the governance digest
in registry state. Root-only transitions and root rotations then fail closed.

The selected-ledger loader admits exactly one legacy transition, root rotation,
or threshold transition per generation. It keeps governance and root history
role-disjoint from every observer key, while the new self-contained report
replays all three event types and the exact v1.492 observer-trust report. The
...
The released v1.504.0 milestone extends that boundary to a configurable receipt
...
receipt kind, receipt, checkpoint, request, response, signer absence, and
witness outcome before it reads private signing material.

The resulting checkpoint keeps the existing approval-log wire format and
signature domain. Output is alias-checked and no-clobber. The gate does not
replay raw history, receipts, responses, or witness signatures at signing time,
protect retained files or keys, or establish trusted time, independent
operation, global publication/non-equivocation, ordering, payment, or
exactly-once execution.

The released v1.506.0 milestone adds a closed dedicated checkpoint and
...
The released v1.507.0 milestone independently witnesses that dedicated
checkpoint. Every witness re-runs the v1.506 report, log, suffix, signature,
and trusted-key checks before private-key access, then signs the exact
checkpoint and log state beneath a separate witness domain.

Verification requires 2–100 fresh, distinct non-weak witness identities and
keys, rejects reuse of the checkpoint signing key, canonicalizes the accepted
identity and key sets, and retains a valid below-threshold report before
returning nonzero. All public and secret inputs are re-read before alias-free,
no-clobber publication.

Direct key pins do not prove separate people, organizations, systems, or key
custody. Protected storage, trusted time, global publication/non-equivocation,
legal identity, ordering, payment, and exactly-once execution remain false.

The released v1.508.0 milestone adds generation-chained trust without changing
the v1.507 witness or quorum-report wire contracts. Generation-zero state pins
one witness identity to one non-weak Ed25519 key. Every transition advances
exactly one generation, commits to the preceding rotation digest and a
nondecreasing bounded timestamp, and requires signatures from both the retained
and successor keys over one factory-specific payload.

Application verifies the complete predecessor relationship and both signatures
before publishing a new immutable trust snapshot. Quorum verification accepts
either paired direct key files or paired current trust states for the whole
invocation, never both, while retaining the unchanged freshness, distinctness,
and checkpoint-signer role-separation rules. Old witnesses fail current trust;
new witnesses fail stale trust.

This detects replay, gaps, mutation, and stale-key substitution only when the
consumer preserves the current state. Protected storage, trusted time,
independent operators or key custody, global publication/non-equivocation,
legal identity, ordering, payment, and exactly-once execution remain false.

The released v1.509.0 milestone closes the transport gap without changing the
v1.507 witness or quorum formats. It validates endpoint and key configuration,
parses the complete public inputs, and re-runs the production v1.506 verifier
before it reads an optional Bearer token or sends one bounded no-redirect HTTPS
request.

The canonical response must satisfy the unchanged signature, evidence,
freshness, and checkpoint-signer role-separation checks under either a direct
key pin or the current v1.508 trust state. pcbex then re-reads every local input
and publishes the witness with a closed receipt that binds raw and semantic
input digests, endpoint, request, response, selected keys/trust generation, and
evaluation time. The credential is never retained.

This proves one configured witness key signed the exact locally verified
checkpoint and that those bytes arrived through one bounded exchange. Protected
files, trust states, and keys; trusted time; endpoint legal identity or
...
request, response, and witness identities into one immutable successor event.
...
structural boundary: it does not replay the receipt-quorum report, complete
approval log, dedicated checkpoint, exact response, witness signature,
freshness, or current trust evidence.

The released v1.511.0 milestone adds a dedicated verifier-bound admission path.
...
same-principal files.

The released v1.512.0 milestone adds distinct multi-receipt admission quorum.
It verifies the shared v1.504 report/log and v1.506 checkpoint once, binds each
v1.509 receipt to its exact v1.507 response and one uniform direct-key or
v1.508 trust-state mode, then invokes the production witness-quorum verifier
once. Duplicate receipt, response, witness, identity, and key evidence fail
closed.

Successful members are sorted by witness identity before the unchanged v1.510
events are appended. A new canonical report binds that exact suffix and the
complete resulting admission log, and both outputs publish without overwrite
only after every input is re-read by identity, byte count, and SHA-256.
Protected state and keys, trusted time, atomic same-principal snapshots or a
...
The released v1.513.0 milestone signs only the exact admission snapshot bound by
one successful v1.512 report. It validates the complete approval-log chain,
matches the report's ID, entry count, head, and canonical digest, and checks the
sorted v1.510 suffix before private-key access.

The command emits the existing generic approval checkpoint, then re-reads the
log, report, and key before alias-free no-clobber publication. It does not
replay raw receipts, responses, report inputs, the dedicated checkpoint,
witness signatures, or trust states at signing time. Protected state or keys,
a dedicated signature domain, trusted time, independent operation, global
publication/non-equivocation, ordering, payment, and exactly-once execution
remain false.
...
admission-log state, threshold/result, and signer beneath a new
checkpoint-witness receipt-quorum domain.

Signing repeats the strict v1.513 public-evidence gate before private-key
access. Signing and verification re-read every input before one canonical,
alias-free, no-clobber output is published. Generic approval checkpoints and
v1.506 factory receipt-quorum signatures cannot substitute.

The checkpoint does not replay raw v1.512 receipt, response, witness-signature,
or trust-state evidence. It also does not protect files or keys, establish
...
The released v1.515.0 milestone adds closed witness and quorum-report contracts
...
The released v1.516.1 milestone preserves those witness and quorum-report bytes
while binding each final witness identity to a generation-zero trust state.
Each successor transition advances exactly one generation and commits to its
predecessor digest; both the retained and successor Ed25519 keys sign one
dedicated-domain payload before the new key becomes current.

The unchanged v1.515 verifier now accepts one uniform trust mode: paired direct
keys or paired current trust states. It rejects stale and successor-key
mismatches, mixed modes, identity mismatch, repeated keys, weak keys, and
checkpoint-signer role reuse. Every CLI input is re-read before alias-free
no-clobber publication.

This boundary does not protect retained files or keys, establish trusted time,
prove independent key custody or operators, provide global
publication/non-equivocation, establish legal identity, or prove ordering,
payment, or exactly-once execution.

The v1.516.1 patch also raises the bounded release-build job ceiling from 45 to
60 minutes after two clean Windows release builds exhausted the earlier limit.
...
reads an optional Bearer token or sends one bounded no-redirect HTTPS request.

The canonical response must pass the unchanged signature, exact evidence,
identity, 24-hour freshness, and checkpoint-signer role-separation checks under
either a direct key pin or current v1.516 trust state. pcbex then re-reads every
local input and publishes the unchanged witness with a closed receipt binding
raw and semantic evidence digests, endpoint, request, response, keys, trust
generation, and evaluation time. The credential is never retained.

The dedicated validator replays every retained input, the exact response, and
current trust entirely offline before emitting the byte-identical normalized
receipt. This proves one configured key signed one exact locally verified
checkpoint and that those bytes crossed one bounded exchange. It does not
...
The released v1.519.0 milestone publishes one unchanged successful transport
...
Admission reconstructs the compact v1.517 request, matches every semantic and
raw source binding, verifies the exact v1.515 response, then enforces direct or
current v1.516 trust, identity, freshness, role separation, and the Ed25519
signature. Every input is re-read by identity, byte count, and SHA-256 before
...
The released v1.521.0 milestone extends that replay boundary to a configurable
...
The output remains the existing generic approval checkpoint, so established
`verify-approval-log`, anchoring, consistency, gossip, and witness controls stay
byte-compatible. The log, report, and private key are re-read before one
alias-free no-clobber publication.

Signing does not replay raw receipts, responses, report inputs, the dedicated
checkpoint, final-witness signatures, or trust states; v1.521 owns those
...
The released v1.523.0 milestone closes the domain gap while retaining the
...
Witnesses sign beneath a new final-checkpoint witness domain. Quorum
verification requires 2–100, enforces a 24-hour window, sorts distinct
identities and keys, rejects weak keys and checkpoint-signer key reuse, and
retains a valid below-threshold report before returning nonzero.

This proves signatures from the configured keys, not real operator
...
The released v1.525.0 milestone closes that rotation gap without changing the
...
One bounded no-redirect HTTPS POST requests one expected identity and accepts
only one unchanged canonical v1.524 witness. The response must pass the
production signature, 24-hour freshness, evidence, identity, key, and role
checks before a distinct no-clobber witness/receipt set is published.

The closed credential-free receipt binds the raw and semantic public evidence,
compact request, raw response, selected keys, and optional v1.525 trust-state
digest and generation. It records the declared endpoint and evaluation time;
offline validation reconstructs the request and repeats that declared-time
local decision without reading the current clock or contacting the endpoint.

This proves one configured key signed the accepted final checkpoint evidence;
it does not prove historical transport behavior, trusted time, endpoint legal
