...
The complete package validator accepts only the canonical single-disk classic
ZIP framing emitted by pcbex. ZIP64, encrypted entries, data descriptors,
local or central extra fields, non-regular entry modes, self-extracting/archive
prefixes, gaps, overlaps, and orphan local records fail closed. It walks the
bounded central directory before the ZIP library, requires every local header
to carry the same raw UTF-8 name and metadata, and then binds the entry count,
archive offset, and central-directory offset to the library's parsed view.
Stored and raw-DEFLATE entries are the only accepted compression methods. A
DEFLATE stream must terminate after consuming exactly its declared compressed
range and must expand to exactly its declared size; declarations are charged
against the 1 MiB manifest, 128 MiB per-entry, and 512 MiB artifact-total
limits before expansion. This prevents duplicate-name collapse, alternate
local/central names, or opaque bytes after a compressed stream from evading
the entry, size, and safe-name contracts.
...
## Exact final-BOM verification (v1.464)

`verify-final-bom` checks one fully validated package against one exact KiCad
board without regenerating Gerbers or invoking KiCad:
...
pcbex final-bom-report-schema --output final-bom-report-v1.schema.json
```

The verifier runs the complete package validator, extracts the board's exact
manufacturing-part inventory, and regenerates `bom.csv` with the canonical
renderer used by `fabricate`. Approval requires both byte-for-byte equality
between that regenerated BOM and the ZIP entry and equality between the
supplied board's byte/SHA-256 identity and the package manifest's input
descriptor. `sources.bom` records the actual ZIP entry and
`sources.canonical_bom` records the regenerated bytes, so a rejected comparison
retains both identities. Equivalent CSV rows with different ordering, quoting,
or line endings are not canonical and remain rejected.

The path-free report has schema version 1 and scope
`final_bom_source_and_canonical_bom_v1`. It records identities for the board,
package, manifest, actual and canonical BOMs, and package board source; total
and in-BOM counts; and at most 256 reference-sorted populated parts with exact
`reference`, `value`, `footprint`, nullable `mpn`, `layer`, and `type` fields.
The only retained findings are `canonical_bom_mismatch` and
`package_board_source_mismatch`; `approved` is true exactly when neither is
present. There is no assembly quantity or placement-coordinate field.
Board and package total-part counts remain independently reported; approval
does not compare excluded/DNP/mechanical footprint totals.

The report's `board_basename` is informational. The package manifest's input
basename is deliberately neither emitted nor approval-bound: source approval
compares exact byte count and SHA-256 only. Both caller inputs are reread before
atomic no-clobber publication. A valid mismatch is published before the
optional final `--require-approved` gate fails; malformed or unsafe board or
package input is a hard error and produces no report.

This is a source-and-canonical-BOM check, not an electrical binding. It does
not prove circuit/schematic connectivity, pin mapping, routing, DRC/DFM,
current supplier facts, or fabrication readiness, and does not authorize
procurement or ordering. The optional downstream offline catalog composition
